...
flgDisp = 1;
% максимальное отклонение реального момента от среднего
stdRange = 1;
...
if flgDisp
    ...
    for i=1:1:5
        myplot([ j1(start:finish) j2(start:finish) j3(start:finish) j4(start:finish) j5(start:finish) tArr(start:finish,i) meanT(start:finish,i)],{'$j_1$','$j_2$','$j_3$','$j_4$','$j_5$',sprintf('$\\tau_%d~$',i),sprintf('$\\overline{\\tau_%d}$',i)}, sprintf('..\\imgs\\calculated\\meanT_%d.jpg',i),'northwest',sprintf('CalculatedTorques for joint %d Std range %.2f',i,stdRange));
    end;
    close all;
end;
